...
import org.openmuc.framework.server.restws.json.JsonArray;
import org.openmuc.framework.server.restws.json.JsonFieldInformation;
import org.openmuc.framework.server.restws.json.JsonObject;
import org.openmuc.framework.server.restws.json.JsonObjectMap;
import org.openmuc.framework.server.restws.json.JsonReader;
import org.openmuc.framework.server.restws.json.JsonTextType;
...
public class JsonHelper {
	...
	public static Value JsonToValue(String jsontext) {
		...
		while (iter.hasNext()) {
			...
			if (jfi.getFieldName().equals("value") && jfi.getFieldJsonTextType().equals(JsonTextType.JsonNumber)) {
		...
		while (iter.hasNext()) {
			...
			if (jfi.getFieldName().equals("value") && jfi.getFieldJsonTextType().equals(JsonTextType.JsonNumber)) {
				...
				++i;
					...
					&& ((jfi.getFieldJsonTextType().equals(JsonTextType.JsonString)) || (jfi.getFieldJsonTextType()
	...
	private static JsonObject ChannelRecordToJson(Channel ch, Record rc) {
		...
		jst.writeObjectField("description");
		jst.writeObjectValue(ch.getDescription());
	...
	public static String JsonToConfigValue(String jsontext) {
		...
		while (iter.hasNext()) {
			...
			if (jfi.getFieldName().equals("value")
					&& (jfi.getFieldJsonTextType().equals(JsonTextType.JsonNumber) || jfi.getFieldJsonTextType()
